...
public class StatusBarService extends Service implements CommandQueue.Callbacks {
	...
	private void makeStatusBarView(Context context) {
		...
		try {
			mCurrentBrightness = Settings.System.getInt(getContentResolver(), Settings.System.SCREEN_BRIGHTNESS);
			mSeekbarBrightness.setProgress((int)mCurrentBrightness);
			changeBrightnessIndicator((int)mCurrentBrightness);
		...
		mSeekbarBrightness.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
			...
			public void onStopTrackingTouch(SeekBar seekBar) {
				try {
					IPowerManager.Stub.asInterface(ServiceManager.getService("power")).setBacklightBrightness(progress);
					Settings.System.putInt(getContentResolver(), Settings.System.SCREEN_BRIGHTNESS, progress);
				}catch (Exception e){
					Slog.w("ucupui", "Setting Brightness failed: " + e);
				}
			}
			...
			public void onProgressChanged(SeekBar seekBar, int progress,
				...
				changeBrightnessIndicator(progress);
			}
		});
	...
	public void changeBrightnessIndicator(int brightness){
		int temp = 255 / 2;
		int temp2 = temp / 2;
		if(brightness >= temp2 && brightness <= temp + temp2){
			mIndicatorBrightness.setImageResource(R.drawable.settings_ic_appwidget_settings_brightness_half_holo);
		}else if(brightness <= temp2){
			mIndicatorBrightness.setImageResource(R.drawable.settings_ic_appwidget_settings_brightness_off_holo);
		}else{
